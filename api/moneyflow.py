"""
Money Flow Data Acquisition from Tushare Pro
- Fetches daily money flow (capital flow by order size) for all stocks
- Saves one file per trading date: stock_data/moneyflow/moneyflow_YYYYMMDD.csv
- Skips dates that already have a local file (incremental)

Usage:
    from api.moneyflow import run

    run('download')                                         # catch up to yesterday
    run('range', start_date='20181024', end_date='20181130') # backfill a range
...
def _existing_dates():
    return {f.stem.replace('moneyflow_', '') for f in DATA_DIR.glob('moneyflow_*.csv')}
...
def _download_one_date(date):
    """Fetch moneyflow for a single date. Returns (date, n_rows, ok|fail)."""
    pro = _get_pro()
    df  = _fetch_with_retry(pro.moneyflow, trade_date=date, fields=','.join(FIELDS))
    if df is not None and not df.empty:
        fp = DATA_DIR / f'moneyflow_{date}.csv'
...
def download_range(start_date, end_date, skip_existing=True):
    """
    Download moneyflow for every trading day in [start_date, end_date].
    Uses parallel workers for speed; safe to interrupt and re-run.
    """
    _setup()
...
def status():
    ...
    if not existing:
        print("No moneyflow data downloaded yet.")
        return
    print(f"moneyflow/ has {len(existing)} date files")
    print(f"  Earliest: {existing[0]}")
    print(f"  Latest:   {existing[-1]}")

    # Check for gap vs daily_basic
    db_dir = Path('./stock_data/daily_basic')
    if db_dir.exists():
        db_dates = {f.stem.replace('daily_basic_', '') for f in db_dir.glob('daily_basic_*.csv')}
        mf_dates = set(existing)
        missing  = sorted(db_dates - mf_dates)
        if missing:
...
def run(action='download', **kwargs):
    """
    Entry point for moneyflow data operations.
    ...
    Keyword args:
        start_date  (str)  YYYYMMDD – used with 'range'
        end_date    (str)  YYYYMMDD – used with 'range'
