...
Pass rate on the 5-question grounding eval: **4 / 5 (80 %)**.
...
│   └── predict.py            # → news_linked.parquet (491k articles, 25.2% coverage)
├── rag/
│   ├── retriever.py          # alias resolution + ts_code-filtered news
...
| 5 | `新能源车板块龙头是谁，业绩对比如何？` | ✗ (no semantic retrieval) |

Q5 fails honestly — the alias dict no longer false-matches "新能源" against
unrelated 能源 stocks; instead it returns "未能识别出A股代码". Fixing this
properly needs Phase 2 (bge-m3 dense retrieval over the entity index).

---

## Known limitations

- **No semantic retrieval yet.** Queries with no direct stock name or symbol
  ("新能源车板块龙头", "白酒龙头") cannot be resolved. Phase 2 in the original
  plan adds bge-m3 + entity FAISS index to handle these.
